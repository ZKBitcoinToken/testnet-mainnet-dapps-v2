...
-->
	<div class="balance" v-if="selectedToken">
        <p>
          Your zkSync Era Account: <span>{{ userAccountsss }} </span>
        </p>
        <p>
          Your zkSync Era ETH: <span>{{ currentBalance }} {{ selectedToken.symbol }}</span>
        </p>
        
      </div>  
    <h1> zkBitcoin Staking ETH Dapp, v1.1.3</h1>
    <h2>Greeter says: {{ greeting }} 👋</h2>
<h2>Your Unstaked and Staked Tokens are worth a total {{ (parseInt(ReservesTotal)/1e18* (parseInt(greetingStakeBalance)+parseInt(MaxsliderValue2)) / greetingLPTotal) }} ETH and {{  (parseInt(ReservesTotal2)/1e18* (parseInt(greetingStakeBalance)+parseInt(MaxsliderValue2)) / greetingLPTotal) }} zkBTC</h2>
...
<h3>Tokens to deposit: {{ parseFloat(sliderValue2).toFixed(12).replace(/(\.0+|(?:(\.\d+?)0+)(?=\D|$))$/, '$2') }} LP Tokens</h3>
     <button
          class="my-button"
          :disabled="!selectedToken || txStatus != 0"
 ...
 <button
    ...
    <h2>Your Staked Tokens are worth {{ (parseInt(ReservesTotal)/1e18* greetingStakeBalance / greetingLPTotal) }} ETH and {{  (parseInt(ReservesTotal2)/1e18* greetingStakeBalance / greetingLPTotal) }} zkBTC</h2>
...
<h3>Tokens to withdraw: {{ parseFloat(sliderValue).toFixed(12).replace(/(\.0+|(?:(\.\d+?)0+)(?=\D|$))$/, '$2') }} LP Tokens</h3>
        ...
        >
          <span v-if="selectedToken && !txStatus">Click this to withdraw {{ parseFloat(sliderValue).toFixed(12).replace(/(\.0+|(?:(\.\d+?)0+)(?=\D|$))$/, '$2')}} LP Tokens to Withdraw</span>
          <span v-else-if="!selectedToken">Select token to pay fee first</span>
          <span v-else-if="txStatus == 1">Sending tx...</span>
          <span v-else-if="txStatus == 2"
            >Waiting until tx is committed...</span
...
<h1>LP Tokens Total Supply {{ (parseInt(greetingLPTotal)/1e18)}} LP Tokens</h1>
  ...
  <h2>Pool is worth {{ (parseInt(ReservesTotal)/1e18) }} and {{  (parseInt(ReservesTotal2)/1e18) }} </h2>
  
    <h2>Total ETH in Pool = {{ (parseInt(ReservesTotal)/1e18) }} ETH </h2>

    <h2>Total zkBitcoin in Pool = {{ (parseInt(ReservesTotal2)/1e18) }} zkBTC </h2>

<h1> Percent of Total Liquidity Staked for Pool {{ (greetingStakeBalanceTotal / greetingLPTotal*100 ).toFixed(4) }} % </h1>


    <h2>Your Total ETH in Pool ={{ (parseInt(ReservesTotal)/1e18* greetingStakeBalance / greetingLPTotal) }} ETH </h2>

    <h2>Your Total zkBitcoin in Pool = {{ (parseInt(ReservesTotal2)/1e18* greetingStakeBalance / greetingLPTotal) }} zkBTC </h2>
...
const LP_CONTRACT_ADDRESS = "0xd0aBECf11E0293800D8E37ad9D644E17d8da4A12"
const Staking_zkBTC_CONTRACT_ADDRESS = "0x31de1Ab036507899780b73355e6cf7601A98d7d4"
...
const LP_SECOND_CONTRACT_ADDRESS = "0x2e7d7F7ff586712915Ff16F214dA6483c5Be65fE"
const LP_SECOND_STAKING_CONTRACT_ADDRESS = "0xAe70D2A183E013651350270313ee908727c68024"
