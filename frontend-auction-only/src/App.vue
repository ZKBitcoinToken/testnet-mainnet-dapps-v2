...
--> <div class="balance" v-if="selectedToken">
    ...
    <h1> zkBitcoin Auctions Dapp, v1.1.3</h1>
    <h2>Greeter says: {{ greeting }} 👋</h2>
<!---
<h2>Your Unstaked and Staked Tokens are worth a total {{ (parseInt(ReservesTotal)/1e18* (parseInt(greetingStakeBalance)+parseInt(MaxsliderValue2)) / greetingLPTotal) }} and {{  (parseInt(ReservesTotal2)/1e18* (parseInt(greetingStakeBalance)+parseInt(MaxsliderValue2)) / greetingLPTotal) }} </h2>
...
<p>Selected slider value: {{ sliderValue2 }}</p>
      ...
      <button
        ...
        >
          <span v-if="shouldDisplayButton1 && selectedToken && !txStatus">Click this to permit Approve LP Tokens to the Staking <br>and Stake {{ sliderValue2 }} LP Tokens in <br>one transaction</span>
          <span v-else-if="!selectedToken">Select token to pay fee first</span>
          <span v-else-if="txStatus == 1">Sending tx...</span>
          <span v-else-if="txStatus == 2"
            >Waiting until tx is committed...</span
	...
	<button
        ...
        >
          <span v-if="shouldDisplayButton1 && selectedToken && !txStatus"> or Click this to approve {{ sliderValue2}} LP Tokens to the Staking Contract in<br> 2 transactions</span>
          <span v-else-if="!selectedToken">Select token to pay fee first</span>
          <span v-else-if="txStatus == 1">Sending tx...</span>
          <span v-else-if="txStatus == 2"
            >Waiting until tx is committed...</span
...
<h1>LP Tokens Total Supply {{ (parseInt(greetingLPTotal)/1e18)}} LP Tokens</h1>
  ...
  <h2>Pool is worth {{ (parseInt(ReservesTotal)/1e18) }} and {{  (parseInt(ReservesTotal2)/1e18) }} </h2>
    ...
    <h2>Total Asset #1 in Pool = {{ (parseInt(ReservesTotal)/1e18) }} Asset #1 </h2>

    <h2>Total Asset #2 in Pool = {{ (parseInt(ReservesTotal2)/1e18) }} Asset #2 </h2>

<h1> Percent of Total Liquidity Staked for Pool {{ (greetingStakeBalanceTotal / greetingLPTotal*100 ).toFixed(4) }} % </h1>


    <h2>Your Total Asset #1 in Pool ={{ (parseInt(ReservesTotal)/1e18* greetingStakeBalance / greetingLPTotal) }} Asset #1 </h2>

    <h2>Your Total Asset #2 in Pool = {{ (parseInt(ReservesTotal2)/1e18* greetingStakeBalance / greetingLPTotal) }} Asset #2 </h2>
    <h2>Your Staked Tokens are worth {{ (parseInt(ReservesTotal)/1e18* greetingStakeBalance / greetingLPTotal) }} and {{  (parseInt(ReservesTotal2)/1e18* greetingStakeBalance / greetingLPTotal) }} </h2>
...
--->
        <br><br>
        <h1>Current Auction Information</h1>
<!---
Input ETH to Send:
<input
  class="form-control"
...
Formating stuff here{{ parseFloat(currentAuctionTotal / 1e18).toFixed(18).replace(/(\.0+|(?:(\.\d+?)0+)(?=\D|$))$/, '$2') }}
--->
   ...
   <h3>If you bid {{number2}} Ethereum you will recieves {{ (( (parseFloat(number2)+parseFloat(UserTotal)/1e18)) / (parseFloat(currentAuctionTotal)/1e18+(parseFloat(number2))) * 32768).toFixed(5) }} zkBitcoin, if you bid {{ number2 }} Ethereum below and if the auction ends with a dust bid.</h3>
    ...
    Input ETH to BID: <input class="form-control" type="number" id="number2" v-model="number2" placeholder="0.01" :step="StepEthToSend" :max="maxQuantity" min=0>
    <h1>Total ETH TO SEND Contract is: {{ message }} ETH</h1>
...
Input Total ETH to Spend: <input
  class="form-control"
  type="number"
  id="number2"
  v-model="number2"
...
<br>
  ...
  Fun Command, change the greeting on top of webpage
...
<Header />
    <div class="start-screen">
      ...
      <h1>Welcome to zkBitcoin Auctions dApp! Connect Metamask Below to enter</h1>
...
console.log("this.amountOfDays", this.amountOfDays);
console.log("this.StartingDay", this.StartingDay);
      var totalDays = parseInt(this.amountOfDays) + parseInt(this.StartingDay);
...
console.log("this.era", this.era);
      ...
      if(parseInt(this.currentEra) < parseInt(this.era)){
			...
			if(parseInt(this.amountOfDays)<150){
				this.StartingDay = 150 - parseInt(this.amountOfDays);
...
console.log("222212", userAddy );
    ...
    try {
        ...
        this.StakeReward3 = await this.getStakeReward3();
        this.retreivingFee = true;
        this.AuctionLengthInHours = await this.getGreetingAuctionLength();
        this.LPBalance = await this.getLPBalance();
        this.ApprovedLPforContract = await this.getApprovedLPforContract();
        this.retreivingBalance = true;
        // Update balance and fee
