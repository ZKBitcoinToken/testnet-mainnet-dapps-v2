<template>
  <div id="app" v-if="!mainLoading">
    <Header />
    <h1>2Download Testnet Miner for zkBitcoin @ <a href="https://github.com/Untouchable2k/zkBTC-Miner-Windows/tree/main">Click here to download Windows GPU and CPU Miner</a></h1>
    <h1>Get ZK Sync Testnet ETH @ <a href="https://faucet.triangleplatform.com/zksync/testnet?">CLICK HERE TO GO TO ETH FAUCET</a></h1>
    <h1>  <a :href="URL_LP_CONTRACT_ADDRESS2">Get 0xBTC/zkBTC LP Tokens by depositing to the LP Click here</a></h1>
...
-->    
	<div class="balance" v-if="selectedToken">
        <p>
          Your ETH Account: <span>{{ userAccountsss }} </span>
        </p>
        <p>
          Your ETH: <span>{{ currentBalance }} {{ selectedToken.symbol }}</span>
        </p>
        
      </div>  
    <h1> zkBitcoin Staking 0xBitcoin Dapp, v1.1.3</h1>
    <h2>Greeter says: {{ greeting }} 👋</h2>

<h2>Your Unstaked and Staked Tokens are worth a total {{ (parseInt(ReservesTotal)/1e8* (parseInt(greetingStakeBalance)+parseInt(MaxsliderValue2)) / greetingLPTotal).toFixed(8) }} 0xBTC and {{  (parseInt(ReservesTotal2)/1e18* (parseInt(greetingStakeBalance)+parseInt(MaxsliderValue2)) / greetingLPTotal) }} zkBTC</h2>
<h1>Depositing</h1>
 <h2>Your LP Tokens {{  parseFloat((MaxsliderValue2/1e18)).toFixed(12).replace(/(\.0+|(?:(\.\d+?)0+)(?=\D|$))$/, '$2') }} LP Tokens </h2>
...
<p>Selected slider value: {{ parseFloat(sliderValue2).toFixed(13).replace(/(\.0+|(?:(\.\d+?)0+)(?=\D|$))$/, '$2') }} </p>
      <button
        ...
        >
          <span v-if="shouldDisplayButton1 && selectedToken && !txStatus">Click this to permit Approve LP Tokens to the Staking <br>and Stake {{ parseFloat(sliderValue2).toFixed(13).replace(/(\.0+|(?:(\.\d+?)0+)(?=\D|$))$/, '$2') }} LP Tokens in <br>one transaction</span>
          <span v-else-if="!selectedToken">Select token to pay fee first</span>
          <span v-else-if="txStatus == 1">Sending tx...</span>
          <span v-else-if="txStatus == 2"
            >Waiting until tx is committed...</span
	...
	<button
        ...
        >
          <span v-if="shouldDisplayButton1 && selectedToken && !txStatus"> or Click this to approve {{ parseFloat(sliderValue2).toFixed(13).replace(/(\.0+|(?:(\.\d+?)0+)(?=\D|$))$/, '$2') }} LP Tokens to the Staking Contract in<br> 2 transactions</span>
          <span v-else-if="!selectedToken">Select token to pay fee first</span>
          <span v-else-if="txStatus == 1">Sending tx...</span>
          <span v-else-if="txStatus == 2"
            >Waiting until tx is committed...</span
 ...
 <button
        ...
        >
          <span v-if="!shouldDisplayButton1 && selectedToken && !txStatus">Step 2) Click this to deposit {{ parseFloat(sliderValue2).toFixed(13).replace(/(\.0+|(?:(\.\d+?)0+)(?=\D|$))$/, '$2')}} LP Tokens to the Staking Contract</span>
          <span v-else-if="!selectedToken">Select token to pay fee first</span>
          <span v-else-if="txStatus == 1">Sending tx...</span>
          <span v-else-if="txStatus == 2"
            >Waiting until tx is committed...</span
...
<h2>Your Staked Tokens are worth {{ (parseInt(ReservesTotal)/1e8* greetingStakeBalance / greetingLPTotal).toFixed(8) }} 0xBTC and {{  (parseInt(ReservesTotal2)/1e18* greetingStakeBalance / greetingLPTotal) }} zkBTC </h2>
...
<h3>Selected tokens to withdraw: {{ parseFloat(sliderValue).toFixed(12).replace(/(\.0+|(?:(\.\d+?)0+)(?=\D|$))$/, '$2') }} LP Tokens</h3>
        ...
        >
          <span v-if="selectedToken && !txStatus">Click this to withdraw {{ parseFloat(sliderValue).toFixed(13).replace(/(\.0+|(?:(\.\d+?)0+)(?=\D|$))$/, '$2')}} LP Tokens to Withdraw</span>
          <span v-else-if="!selectedToken">Select token to pay fee first</span>
          <span v-else-if="txStatus == 1">Sending tx...</span>
          <span v-else-if="txStatus == 2"
            >Waiting until tx is committed...</span
...
<!--
    <div class="balance" v-if="selectedToken">
        <p>
          Your ETH Account: <span v-if="retreivingBalance">Loading...</span>
...
<h1> POOL STATS Extra Info </h1>
<h1>LP Tokens Total Supply {{ (parseInt(greetingLPTotal)/1e18)}} LP Tokens</h1>

  <h2>Pool is worth {{ (parseInt(ReservesTotal)/1e8) }} 0xBTC and {{  (parseInt(ReservesTotal2)/1e18) }} zkBTC</h2>
  
    <h2>Total 0xBTC in Pool = {{ (parseInt(ReservesTotal)/1e8).toFixed(8) }} 0xBTC </h2>

    <h2>Total zkBTC in Pool = {{ (parseInt(ReservesTotal2)/1e18) }} zkBTC </h2>

<h1> Percent of Total Liquidity Staked for Pool {{ (greetingStakeBalanceTotal / greetingLPTotal*100 ).toFixed(4) }} % </h1>


    <h2>Your Total 0xBTC in Pool ={{ (parseInt(ReservesTotal)/1e8* greetingStakeBalance / greetingLPTotal).toFixed(8) }} 0xBTC</h2>

    <h2>Your Total zkBTC in Pool = {{ (parseInt(ReservesTotal2)/1e18* greetingStakeBalance / greetingLPTotal) }} zkBTC </h2>
...
<h2>Your Approved LP Tokens for Contract {{  ApprovedLPforContract / 1e18 }}% Approved LP Tokens</h2>
  ...
  <div id="app" v-else>
    ...
    <div class="start-screen">
      ...
      <h1>Welcome to 0xBitcoin Staking dAPP for zkBitcoin! Connect Metamask Below to enter</h1>
...
// 0xBTC POOL ONLY Main Contracts Below
const LP_CONTRACT_ADDRESS = "0x2e7d7F7ff586712915Ff16F214dA6483c5Be65fE"
const Staking_zkBTC_CONTRACT_ADDRESS = "0xAe70D2A183E013651350270313ee908727c68024"
const zkBTC_CONTRACT_ADDRESS = "0xe28CA0f2E8D2Ea38E7E0203F491Ae8Ea8d6B188F"
const ZeroxBitcoin_addresss = "0xa0E1d12603EA3E256aA52AaB96271e97dC66ED7a"
const AUCTION_CONTRACT_ADDRESS = "0x368Cdd081e3Ab6369B3E052dF3a364400bbf863A"

//SECOND STAKING ADDRESS
const LP_SECOND_CONTRACT_ADDRESS = "0xd0aBECf11E0293800D8E37ad9D644E17d8da4A12"
const LP_SECOND_STAKING_CONTRACT_ADDRESS = "0x31de1Ab036507899780b73355e6cf7601A98d7d4"
...
async ONEloadMainScreen22() {
      ...
      const amount = this.message;
       const userAddy = await this.getAddress()
console.log("222212111", userAddy );
    try {
        this.getBalance();
        this.greeting = await this.getGreeting();
        this.GreetingDaysToWithdrawArray = await this.getGreetingDaysToWithdrawArray();
...
console.log("222212", userAddy );
    this.txStatus = 1;
    try {
const amountToStake = (parseFloat((this.sliderValue2*1.00001).toFixed(18)*1e18)).toString(); // or parseFloat(this.sliderValue2)
...
.form-control {
  ...
  font-size: 30px; /* Adjust the width to your preference */
}
.larger-input {
